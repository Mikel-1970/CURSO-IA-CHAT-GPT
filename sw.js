...
async function enhancedHtmlResponse(response){
  ...
  let html=text;
  if(!html.includes('v19-3.css'))html=html.replace('</head>','<link rel="stylesheet" href="./v19-3.css?v=19.5.2">\n</head>');
  if(!html.includes('v19-3-app.js'))html=html.replace('</body>','<script src="./v19-3-app.js?v=19.5.2"></script>\n</body>');
  if(!html.includes('v19-5-admin-hotfix.js'))html=html.replace('</body>','<script src="./v19-5-admin-hotfix.js?v=19.5.2"></script>\n</body>');
  if(!html.includes('v19-5-fix.js'))html=html.replace('</body>','<script src="./v19-5-fix.js?v=19.5.2"></script>\n</body>');
  if(!html.includes('v19-5-login-hotfix.js'))html=html.replace('</body>','<script src="./v19-5-login-hotfix.js?v=19.5.2"></script>\n</body>');
  ...
  event.waitUntil((async()=>{
    ...
    for(const client of clients){
      try{
        ...
        url.searchParams.set('appv','19.5.2');
