const CACHE='curso-ia-v19-5-login-hotfix-r6';
...
async function enhancedHtmlResponse(response){
  ...
  let html=text;
  if(!html.includes('v19-3.css'))html=html.replace('</head>','<link rel="stylesheet" href="./v19-3.css?v=19.5.3">\n</head>');
  if(!html.includes('v19-3-app.js'))html=html.replace('</body>','<script src="./v19-3-app.js?v=19.5.3"></script>\n</body>');
  if(!html.includes('v19-5-admin-hotfix.js'))html=html.replace('</body>','<script src="./v19-5-admin-hotfix.js?v=19.5.3"></script>\n</body>');
  if(!html.includes('v19-5-fix.js'))html=html.replace('</body>','<script src="./v19-5-fix.js?v=19.5.3"></script>\n</body>');
  if(!html.includes('v19-5-login-hotfix.js'))html=html.replace('</body>','<script src="./v19-5-login-hotfix.js?v=19.5.3"></script>\n</body>');
...
self.addEventListener('activate',event=>{event.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(k=>k!==CACHE).map(k=>caches.delete(k)));await self.clients.claim();const clients=await self.clients.matchAll({type:'window',includeUncontrolled:true});for(const client of clients){try{const url=new URL(client.url);url.searchParams.set('appv','19.5.3');await client.navigate(url.toString());}catch(e){}}})());});
