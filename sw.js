const CACHE='curso-ia-v20-review-r5';
const SHELL=['./manifest.webmanifest','./icon-192.png','./icon-512.png','./v19-3.css','./v19-3-app.js','./v19-5-admin-hotfix.js','./v19-5-fix.js','./v19-5-login-hotfix.js','./v20-review-mode.js','./v20-exam-score.js','./v20-version-unify.js'];

async function enhancedHtmlResponse(response){
  const text=await response.text();
  let html=text;
  if(!html.includes('v19-3.css'))html=html.replace('</head>','<link rel="stylesheet" href="./v19-3.css?v=20.0-review.5">\n</head>');
  if(!html.includes('v19-3-app.js'))html=html.replace('</body>','<script src="./v19-3-app.js?v=20.0-review.5"></script>\n</body>');
  if(!html.includes('v19-5-admin-hotfix.js'))html=html.replace('</body>','<script src="./v19-5-admin-hotfix.js?v=20.0-review.5"></script>\n</body>');
  if(!html.includes('v19-5-fix.js'))html=html.replace('</body>','<script src="./v19-5-fix.js?v=20.0-review.5"></script>\n</body>');
  if(!html.includes('v19-5-login-hotfix.js'))html=html.replace('</body>','<script src="./v19-5-login-hotfix.js?v=20.0-review.5"></script>\n</body>');
  if(!html.includes('v20-review-mode.js'))html=html.replace('</body>','<script src="./v20-review-mode.js?v=20.0-review.5"></script>\n</body>');
  if(!html.includes('v20-exam-score.js'))html=html.replace('</body>','<script src="./v20-exam-score.js?v=20.0-review.5"></script>\n</body>');
  if(!html.includes('v20-version-unify.js'))html=html.replace('</body>','<script src="./v20-version-unify.js?v=20.0-review.5"></script>\n</body>');
